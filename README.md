...
- `global_context.yaml`: プロジェクト全体のグローバルコンテキスト
- `process_context.yaml`: プロセス実行に関するコンテキスト
- `mcp_context.yaml`: MCPサーバー固有のコンテキスト
- `unified_metrics.yaml`: 統一メトリクス定義（v1.0.0）
...
2. 自動リカバリー機能
   - エラータイプに基づく判断（critical/non-critical）
   - リソース状態の監視（CPU、メモリ、ディスク）
   - 指数バックオフによるリトライ戦略
